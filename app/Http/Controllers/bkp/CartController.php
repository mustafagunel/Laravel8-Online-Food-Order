...
class CartController extends Controller
{
    ...
    function getCart(){
        ...
        return view('Home.index',['cart'=>$cart,'total'=>$_total,'min'=>$min[0],'settings'=>$settings[0]]);
    }
    ...
    function checkOut(Request $request, $type){
        ...
        if(!Auth::user()){
            ...
            $q2 = 'select * from settings';
            $settings = DB::select($q2);
            return view('Home.index',['cart'=>$cart,'total'=>$_total,'min'=>$min[0],'alert'=>$alert,'settings'=>$settings[0]]);
        }
        ...
        if( $_total < $min->min_cart){
            ...
            $q2 = 'select * from settings';
            $settings = DB::select($q2);
            return view('User.index',['success'=>"Ödeme işlemi başarılı şekilde gerçekleştirildi.",'settings'=>$settings[0]]);
        }
